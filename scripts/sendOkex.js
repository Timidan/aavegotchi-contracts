async function main() {
    ...
    for (let i = 0; i < recipients.length; i++) {
        ...
        const sendrecipients = await dao.mintItems(recipients[i], itemIds, quantities)
        const receipt = await sendrecipients.wait()
        if (!receipt.status) {
            throw Error(`Not Sent: ${sendrecipients.hash}`)
        }
        console.log('Minted item' ,itemIds[i]  , 'and sent to', recipients[i], 'at txn', sendrecipients.hash, i)
    }
